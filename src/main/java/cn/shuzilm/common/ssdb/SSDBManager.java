...
public class SSDBManager {
    private static final Logger log = LoggerFactory.getLogger(SSDBManager.class);
    ...
    private static final String FILE_REDIS_CONFIG = "configs_ssdb_redis.properties";

    private static HashMap<String,SSDBManager> instanceMap = new HashMap<String,SSDBManager>();
    private static volatile SSDBManager ourInstance = null;
    ...
    private SSDBManager(String configFileName) {
    ...
    public static SSDBManager getInstance( String redisConfigName ) {
        if(!instanceMap.containsKey(redisConfigName)){
            SSDBManager ins = null;
            synchronized (lock2) {
                ins = new SSDBManager(redisConfigName);
    ...
    public static SSDBManager getInstance() {
        if (ourInstance == null) {
            synchronized (lock) {
                if (ourInstance == null) {
                    ourInstance = new SSDBManager(FILE_REDIS_CONFIG);
                }
            }
        }
        return ourInstance;
