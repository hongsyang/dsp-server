...
public class RedisManager {
    ...
    private RedisManager(String configFileName) {
    ...
    public static RedisManager getInstance(String redisConfigName ) {
        if(!instanceMap.containsKey(redisConfigName)){
            RedisManager ins = null;
            synchronized (lock2) {
                ins = new RedisManager(redisConfigName);
    ...
    public static RedisManager getInstance() {
        if (ourInstance == null) {
            synchronized (lock) {
                if (ourInstance == null) {
                    ourInstance = new RedisManager(FILE_REDIS_CONFIG);
                }
            }
        }
        return ourInstance;
